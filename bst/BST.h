...
private:
	...
	int displayHeight(BinNodePointer node)const;

	int displayLeafCount(BinNodePointer node)const;
...
{
	return displayLeafCount(myRoot);
}
...
template <typename DataType>
int BST<DataType>::displayHeight(BinNodePointer node)const
{
if (node == NULL)
{
return 0;
...
int left = displayHeight(node->left);
int right = displayHeight(node->right);
...
template <typename DataType>
int BST<DataType>::displayLeafCount(BinNodePointer node)const
{
	if (node)
	{
